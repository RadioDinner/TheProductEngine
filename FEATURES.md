...
| 31 | **Pay-by-phone card capture** (Twilio `<Pay>` IVR → Stripe card on file) — callers key their OWN card into the phone keypad on a dedicated voice number; Twilio's Stripe Pay Connector tokenizes it (digits never touch the operator, this server, or a log) and it's saved to a Stripe customer; a bearer-authed `POST /charge` bills it off-session per order. The PCI-safe replacement for item 29's operator-keys-the-card call-in flow | session 012 | **standalone service + member-account bridge built (session 016)**: the main app now ADOPTS an IVR-saved card automatically (`resolveStripeCustomer` searches Stripe by the caller's phone metadata and stamps the member's `stripeCustomerId`) at auto-top-up time, admin billing, and the admin user view — REQUIRES both deployments to share ONE Stripe account; the service itself still needs its Twilio+PCI deploy (README) |
