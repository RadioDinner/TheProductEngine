...
| 48 | **Test mode** — an admin switch that sends **real ads to your test numbers and nobody else**. Everything else runs exactly as production (real ad number, price, review queue, batching, category filtering), so the test phone shows what a subscriber would have seen; only the audience is narrowed. Ads posted while it is on stay off the public website and are labelled for cleanup. **It expires itself after 4 hours** — a forgotten test mode is a silent outage, since ads keep flowing and every screen reads healthy while the whole list receives nothing | session 021 | **built** (⚠️ migration 9951 for the `is_test` LABEL only — hiding works without it). Enforced in the store layer, not at call sites; filters the real subscriber list rather than inventing recipients, so category prefs/STOP/blocklist behave for real. Email edition suppressed while on. Loud on the dashboard, Settings and `/api/health` |

| 49 | **An ad is charged when it RUNS, not when it is posted** — *"when people create an ad, and have a card on file, I want the confirmation message to include that the card won't be charged until the ad is run. Make the system honor the truth of this message."* Posting quotes a price and reserves it against the member's balance; the batch that carries the ad out to subscribers is what collects. An ad that never runs is never charged, so turning one down returns nothing because nothing was taken | session 023 | **built** (⚠️ migration 9950 — and unlike every other pending migration this one does not degrade to "the feature is off", it degrades to no ad being charged at all) |
| 50 | **Approved, waiting for payment** — *"I approved the ad, but it wasn't paid, I want the status to go to 'approved, pending payment' but I want the message that the seller gets, to remind them to pay up."* An unfunded ad is reviewed like any other and can be approved; it holds its place and goes out on the next batch once the money lands, with no second decision. The approval text asks for the money instead of promising a send. Past 3 ads waiting on money from one number (a setting), further posts are held out of the queue | session 023 | **built** (rides migration 9950) |
| 51 | **Edit the auto replies from an admin tab** — *"I want an admin tab where I can go in and edit the messages and add or remove variables from auto replies, rather than having a code/prompt session. Plus, I can see the messages."* /admin/replies lists all 28 editable messages in the words they will actually use, with their variables: click one to insert it at the cursor, watch the preview and the cost in texts update as you type. A variable you invent is refused by name; phrases the code depends on (STOP, and the substrings that stop a reply repeating itself) cannot be deleted. Only what you rewrite is stored, so an untouched message still follows the code | session 023 | **built** (⚠️ migration 9949 — until pasted the tab shows the wording but cannot save) |
| 52 | **A receipt when the ad goes out** — the seller is texted as their ad actually runs and the money moves, which is what makes "nothing is charged until your ad runs" checkable rather than merely stated. One text per ad; switchable off on Settings | session 021 | **built** (no migration) |
| 53 | **See the labelled SMS picture on /admin/ads** — *"my system is not automatically labeling the 'text' image with the ad number … I want that same image to be on the ADS page of the admin portal, so I can see what was or what will be sent out in the SMS."* The "AD 1024" label is now burned in when the PICTURE ARRIVES rather than while a batch is being composed, and the labelled copy is kept on the ad instead of discarded — so /admin/ads shows the exact picture a subscriber receives, whole and at a size you can read the number on. A picture replaced afterwards makes the stored label read stale, never wrong. Because a labelling failure is otherwise invisible (the batch quietly sends the plain original), /admin/sms-diag gained a self-test that renders a label for real on the live deployment and counts the ink | session 024 | **built** (⚠️ migration 9948 — until pasted the label is still burned into every broadcast picture, it just isn't kept, so the preview reads "not labelled yet") |
