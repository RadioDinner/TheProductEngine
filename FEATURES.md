...
| 48 | **An ad is charged when it RUNS, not when it is posted** — *"when people create an ad, and have a card on file, I want the confirmation message to include that the card won't be charged until the ad is run. Make the system honor the truth of this message."* Posting quotes a price and reserves it against the member's balance; the batch that carries the ad out to subscribers is what collects. An ad that never runs is never charged, so turning one down returns nothing because nothing was taken | session 021 | **built** (⚠️ migration 9951 — and unlike every other pending migration this one does not degrade to "the feature is off", it degrades to no ad being charged at all) |
| 49 | **Approved, waiting for payment** — *"I approved the ad, but it wasn't paid, I want the status to go to 'approved, pending payment' but I want the message that the seller gets, to remind them to pay up."* An unfunded ad is reviewed like any other and can be approved; it holds its place and goes out on the next batch once the money lands, with no second decision. The approval text asks for the money instead of promising a send. Past 3 ads waiting on money from one number (a setting), further posts are held out of the queue | session 021 | **built** (rides migration 9951) |
| 50 | **Edit the auto replies from an admin tab** — *"I want an admin tab where I can go in and edit the messages and add or remove variables from auto replies, rather than having a code/prompt session. Plus, I can see the messages."* /admin/replies lists all 28 editable messages in the words they will actually use, with their variables: click one to insert it at the cursor, watch the preview and the cost in texts update as you type. A variable you invent is refused by name; phrases the code depends on (STOP, and the substrings that stop a reply repeating itself) cannot be deleted. Only what you rewrite is stored, so an untouched message still follows the code | session 021 | **built** (⚠️ migration 9950 — until pasted the tab shows the wording but cannot save) |
| 51 | **A receipt when the ad goes out** — the seller is texted as their ad actually runs and the money moves, which is what makes "nothing is charged until your ad runs" checkable rather than merely stated. One text per ad; switchable off on Settings | session 021 | **built** (no migration) |
