...
| 31 | **Pay-by-phone card capture** (Twilio `<Pay>` IVR → Stripe card on file) — callers key their OWN card into the phone keypad on a dedicated voice number; Twilio's Stripe Pay Connector tokenizes it (digits never touch the operator, this server, or a log) and it's saved to a Stripe customer; a bearer-authed `POST /charge` bills it off-session per order. The PCI-safe replacement for item 29's operator-keys-the-card call-in flow | session 012 | **BUILT IN-APP (session 016)**: the whole call now runs at `/api/voice` (`lib/voice.ts`) — operator phones ring FIRST (whisper announces the caller), an auto-attendant answers only on no-answer (1 = save a card, 2 = voicemail texted to `ADMIN_PHONES`), `<Pay>` tokenizes, and the card is stamped onto the member's account instantly; confirmations go out over the registered Telnyx line. One Stripe account by construction. `pay-by-phone/` is now reference-only. Ops: PCI Mode + Pay Connector + webhook URL + `TWILIO_AUTH_TOKEN`/`VOICE_RING_TO` — see `docs/call-in-card-line.md` |
