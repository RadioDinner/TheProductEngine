# HANDOFF ARCHIVE — the session-by-session record

Every session's narrative, newest first. Split out of `HANDOFF.md` in session
021 because that file was the single hottest thing in the repo (touched by 8 of
12 consecutive commits) and the only thing that actually conflicted when two
parallel sessions merged on 2026-08-21.

**Nothing here was deleted or rewritten in the split — it is the same text,
moved.** `HANDOFF.md` is now live state only and links here.

**Read this when you need the WHY**: why a thing was built the way it was, what
was tried and rejected, what a decision cost. That reasoning is the most
valuable thing in the repo and it is deliberately kept in full rather than
summarized away. Grep it — the section headers name the session and the day.

Per-session detail beyond this also lives in `Session log/<session>/`.

> ⚠️ **This file is HISTORY. It is not maintained.** Statements here were true
> when written and many are now superseded — old migration numbers, prices,
> send windows, "not yet applied" notes, launch-day checklists. For what is
> true NOW, read `HANDOFF.md`. Never act on a claim in this file without
> checking it against the live document or the code first.
...
# Older state and shipped-work records (pre-session 005)

These were the top of `HANDOFF.md` in the early days and are now firmly
historical — the "Current state" block below describes 2026-07-08, when
migrations were still numbered ascending, Stripe was unset and the 10DLC
campaign was pending. The "Remaining work" block is the pre-launch checklist
from that era. Kept verbatim as the record.
