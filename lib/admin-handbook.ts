...
const ENTRIES = {
  ...
  "ads.textPicture": {
    title: "The picture the text carries",
    what: "A picture ad broadcasts exactly ONE photo, and it goes out with the ad number burned into its bottom-right corner — \"AD 1024\". The thumbnail marked texts on each row IS that labelled copy, at the size it is sent, so what you see here is what a subscriber receives. The other thumbnails are the PIC-on-request extras and the website-only ones.",
    why: "The label is what ties a photo arriving on its own to its line in the batch text and to the PIC command — a picture with no number on it is a photo nobody can act on (session 018). Until session 024 that labelled copy was made while the batch was being composed and thrown away immediately, so the only place it had ever been seen was a subscriber's phone: you could not check it before a batch went out, or afterwards. The user asked for it here, in their words: \"I want that same image to be on the ADS page of the admin portal, so I can see what was or what will be sent out in the SMS.\"",
    gotchas: "\"Text picture isn't labelled yet\" is normal for an ad posted before session 024 and for one whose picture was replaced since — the batch labels it on the way out either way, and Label the picture … now makes it here and then. If labelling FAILS, the ad still sends: the batch broadcasts the plain original with no number on it. That failure is otherwise invisible, so if a row stays unlabelled after pressing the button, run the picture-label self-test on SMS diagnostics — it renders one for real on the live deployment and says whether the ink landed.",
  },
